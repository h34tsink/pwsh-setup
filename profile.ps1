...
# --- SSH Agent ---
# Ensure ssh-agent service is running so keys are available for git/ssh
$sshAgentSvc = Get-Service ssh-agent -ErrorAction SilentlyContinue
if ($sshAgentSvc -and $sshAgentSvc.Status -ne 'Running') {
